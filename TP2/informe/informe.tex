...
\textbf{20 combos} que, según su propia descripción comercial, combinan
\cita{Cocina, lavavajillas, alacenas, mesadas, bacha, grifería, baldosas y papel
vinílico decorativo}: ocho categorías de producto que, entre todas sus
variantes, suman un catálogo de \textbf{30 artículos} distintos. Cada combo es
una lista cerrada de siete u ocho de esos artículos.
...
conviene el plan que menos lugar ocupa; y la descripción de la sección, que
enumera las ocho categorías que comercializa. El mecanismo se detalla en la
...
\begin{figure}[H]
\centering
\resizebox{\linewidth}{!}{\input{../docs/esquema_proceso.tex}}
...
las tres etapas (Anexo~\ref{sec:anexo}).
...
lugares para las cuatro variantes que existen: nunca pudieron presentar holgura
y \textbf{nunca excluirán un combo por sí solos}. Los cuellos de botella
efectivos son dos: \textbf{mesadas}, con tres lugares para cuatro variantes, y
...
nula por la razón ya señalada: su capacidad coincide con el número de variantes
existentes. El único cuello de botella efectivo que subsiste es el espacio de
lavavajillas y cocinas.
...
\paragraph{Verificación.} Se implementaron cuatro capas de control automático
que se ejecutan antes de aceptar cualquier resultado:

\begin{itemize}
  \item \textbf{46 controles sobre los datos de entrada}: que todo artículo
  integrante de un combo exista en el catálogo, que ningún combo repita
  categoría, las 30 frecuencias de la Tabla~\ref{tab:apariciones} una por una,
  las capacidades y las cotas de la Tabla~\ref{tab:cota_trivial}.
  \item \textbf{25 controles cruzados sobre los óptimos} de los tres escenarios:
  que la variedad no supere la cota analítica, que el stock de la solución final
  sea exactamente el necesario, que la ocupación respete todas las capacidades y
  que la solución figure entre las enumeradas exhaustivamente.
  \item \textbf{10 controles sobre las pruebas de supuestos}, formulados como
  propiedades que cualquier resultado correcto debe satisfacer: ampliar la
  capacidad nunca puede reducir la variedad, incrementar las unidades por combo
  nunca puede aumentarla, ningún criterio de desempate puede perder variedad.
  \item \textbf{Enumeración exhaustiva independiente del solver}, que construye
  el stock requerido por cada conjunto de combos y lo compara directamente
  contra las capacidades.
\end{itemize}

Los controles no verifican que el resultado coincida con el esperado sino
